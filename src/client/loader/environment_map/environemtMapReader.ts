import type { ColorSpace, TextureDataType } from 'three';
import {
  CubeTexture,
  DataTexture,
  FileLoader,
...
export class EnvMapReader {
  ...
  private parseHeader_(buffer: ArrayBuffer) {
    const view = new DataView(buffer, 0, _HEADER_BYTES_);
    this._size = view.getUint16(0);
    this._maxLod = view.getUint8(2);
    this._colorSpace = view.getUint16(3)
      ? LinearSRGBColorSpace
      : SRGBColorSpace;
  ...
  private parseBufferData_(buffer: ArrayBuffer, offsetBytes: number) {
    ...
    for (let i = 0; i < numMips; i++) {
      ...
      for (let j = 0; j < 6; j++) {
        const view = new Uint8Array(buffer, offsetBytes, cpp * size * size);
        const dataTexture = new DataTexture(view, size, size);
  ...
  private convertToRGBABuffer(buffer: ArrayBuffer) {
    const view = new Uint8Array(
      buffer,
      _HEADER_BYTES_,
      buffer.byteLength - _HEADER_BYTES_
    );
    ...
    let n = 0;
    const offset = view.length / stride;
    if (stride === 3) {
  ...
  private parseData_(buffer: ArrayBuffer) {
    ...
    offsetByes = 0;
    const newBuffer = this.convertToRGBABuffer(buffer);
