...
const loadGlbMesh = async (glbName: string, glbUrl: string) => {
    ...
    if (!loadedScene) {
        await renderScene.loadResource(glbName + '.glb', glbUrl,
            (_:string) => {},
...
let fixedResolution: any = undefined;
...
const gui = new GUI()
const glbMenuItems: any[] = Object.assign({}, ...glbMap.map((item: any) => ({[item.name]: item.url})));
...
environmentFolder.add<any>(renderScene, 'environmentRotation', 0, Math.PI*2, 0.01).onChange((value) => {
    if (renderScene.scene.userData?.environmentDefinition) {
        renderScene.scene.userData.environmentDefinition.rotation = value;
    }
});
environmentFolder.add<any>(renderScene, 'environmentIntensity', 0, 5, 0.01).onChange((value) => {
    if (renderScene.scene.userData?.environmentDefinition) {
