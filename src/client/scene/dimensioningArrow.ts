...
export class DimensioningArrow extends Group {
  ...
  constructor(start: Vector3, end: Vector3, parameters?: any) {
    ...
    this.startShaft = new Mesh(
      new PlaneGeometry(2, 1).translate(0, 0.5, 0),
      this.startShaftMaterial
    );
    this.startShaft.onBeforeRender = (
      renderer,
      _scene,
      camera,
      _geometry,
    ...
    this.endShaft = new Mesh(
      new PlaneGeometry(2, 1).translate(0, 0.5, 0),
      this.endShaftMaterial
    );
    this.endShaft.onBeforeRender = (
      renderer,
      _scene,
      camera,
      _geometry,
    ...
    this.startArrow = new Mesh(
      new PlaneGeometry(2, 1).translate(0, 0.5, 0),
      this.startArrowMaterial
    );
    this.startArrow.onBeforeRender = (
      renderer,
      _scene,
      camera,
      _geometry,
    ...
    this.endArrow = new Mesh(
      new PlaneGeometry(2, 1).translate(0, 0.5, 0),
      this.endArrowMaterial
    );
    this.endArrow.onBeforeRender = (
      renderer,
      _scene,
      camera,
      _geometry,
  ...
  private createArrowLabel(text: string, parameters?: any): CSS2DObject {
    ...
    labelObject.center.set(
      parameters.origin?.x ?? 0.5,
      parameters.origin?.y ?? 0.5
    );
  ...
  public setPosition(
    ...
    this.startPosition.copy(
      start.clone().add(direction.clone().multiplyScalar(offsetStart))
    );
    this.endPosition.copy(
      end.clone().add(direction.clone().multiplyScalar(-offsetEnd))
    );
  ...
  public setLabel(text: string) {
    const labelDiv = this.arrowLabel.element.children[0] as HTMLDivElement;
    labelDiv.textContent = text;
    this.labelTextClientWidth =
      labelDiv.clientWidth * this.parameters.deviceRatio;
  }
  ...
  public updateArrowLabel(renderer: WebGLRenderer, camera: Camera) {
    const distanceText = this.startPosition
    ...
    const direction = end.clone().sub(start);
    const angle =
      (Math.atan2(-direction.y, (direction.x * width) / height) * 180) /
      Math.PI;
  ...
  private setModelMatrix(
    ...
    object.applyMatrix4(
      new Matrix4().makeTranslation(start.x, start.y, start.z)
    );
  }

  public updateShaftMaterial(
    startArrow: boolean,
    ...
    if (material instanceof DimensioningArrowShaftMaterial) {
      ...
      material.update({
        ...
        arrowPixelSize: new Vector2(
          this.parameters.arrowPixelWidth,
          this.parameters.arrowPixelHeight
        ),
...
const glslShaftVertexShader = `varying vec2 centerPixel;
...
const glslShaftFragmentShader = `varying vec2 centerPixel;
varying vec2 posPixel;
varying vec2 arrowDir;
  ...
  constructor(parameters?: any) {
    super({
      ...
      uniforms: UniformsUtils.clone(
        DimensioningArrowShaftMaterial.shader.uniforms
      ),
...
const glslArrowVertexShader = `varying vec2 arrowUv;
...
const glslArrowFragmentShader = `varying vec2 arrowUv;
varying vec2 centerPixel;
varying vec2 posPixel;
