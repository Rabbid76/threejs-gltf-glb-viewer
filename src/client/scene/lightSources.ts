...
import type { SceneVolume } from '../renderer/render-utility';
import type { ColorRepresentation, Light, Scene } from 'three';
import {
  AmbientLight,
  Color,
  DirectionalLight,
...
export class LightSources {
  ...
  public setLightSourcesDistances(
    sceneVolume: SceneVolume,
    scaleDistance: boolean
  ) {
    this.lightSourceScale = scaleDistance
      ? sceneVolume.maxSceneDistanceFrom0 * 1.5
      : undefined;
    this.updateLightSources();
  }
...
export class LightSourcesGUI {
  ...
  public addGUI(
    gui: GUI,
    lightControlsUpdate: () => void,
    lightSourcesUpdate: () => void
  ): void {
    gui
      .add<any>(this.lightSources, 'lightControls')
      .onChange(lightControlsUpdate);
    gui
      .add<any>(this, 'lights', ['none', 'default', 'fife'])
      .onChange((value: string) => {
        switch (value) {
          ...
          case 'default':
            this.lightSources.currentLightSourceDefinition =
              LightSources.defaultLightSources;
            break;
          case 'fife':
            this.lightSources.currentLightSourceDefinition =
              LightSources.fifeLightSources;
            break;
        }
  ...
  public updateGUI(): void {
    ...
    this.lightSources.getLightSources().forEach((light) => {
      if (light instanceof AmbientLight) {
        ...
        ambiLight
          .addColor(this.lightColors[this.lightColors.length - 1], 'color')
          .onChange(
            (color: ColorRepresentation) => (light.color = new Color(color))
          );
      }
    });
    this.lightSources.getLightSources().forEach((light) => {
      if (light instanceof DirectionalLight) {
        ...
        const lightFolder = gui.addFolder(
          `Directional Light ${this.lightColors.length - 1}`
        );
        ...
        const lightFolder = gui.addFolder(
          `Rect area Light ${this.lightColors.length - 1}`
        );
        this.lightSourceFolders.push(lightFolder);
        const shadowLight =
